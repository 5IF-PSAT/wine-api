...
def compare_wine(request):
    ...
    aggregated_doc_vector = pd.read_csv(f'{BASE_DIR}/compare_data/aggregated_doc_vector.csv', index_col=0)
    ...
    # Load the KD Tree
    with open(f'{BASE_DIR}/model/wine_composition_weather_tree.pkl', 'rb') as f:
        wine_composition_weather_tree = pickle.load(f)
    with open(f'{BASE_DIR}/model/wine_text_review_tree.pkl', 'rb') as f:
        wine_text_review_tree = pickle.load(f)
    ...
    dist_composition_weather, ind_composition_weather = wine_composition_weather_tree.query(
        input_wine_composition_and_weather, k=nb_wines)

    if have_text_review:
        dist_text_review, ind_text_review = wine_text_review_tree.query(input_wine_text_review, k=nb_wines)
    ...
    wines = []
    for index, row in top_wines_plus_one.iterrows():
